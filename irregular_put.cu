...
void int_thread(size_t size, uint32_t num_messages, uint32_t message_size, uint32_t *rand_pe, uint32_t *rand_locat, int *remote_buffer, int *local_buffer)
{
  for(uint32_t i = TID; i<num_messages; i+=blockDim.x*gridDim.x)
  {
      ...
      nvshmem_int_put(remote_buffer+location, local_buffer+TID*message_size, message_size, rand_pe[i]);
  }
}
...
void int_warp(size_t size, uint32_t num_messages, uint32_t message_size, uint32_t *rand_pe, uint32_t *rand_locat, int *remote_buffer, int *local_buffer)
{
  for(uint32_t i = WARPID; i<num_messages; i+=((blockDim.x*gridDim.x)>>5))
  {
      ...
      nvshmemx_int_put_warp(remote_buffer+location, local_buffer+WARPID*message_size, message_size, pe);
  }
}
...
void int_block(size_t size, uint32_t num_messages, uint32_t message_size, uint32_t *rand_pe, uint32_t *rand_locat, int *remote_buffer, int *local_buffer)
{
  for(uint32_t i = blockIdx.x; i<num_messages; i+=gridDim.x)
  {
      uint32_t location = rand_locat[i]%(size-message_size);
      uint32_t pe = rand_pe[i];
      nvshmemx_int_put_block(remote_buffer+location, local_buffer+blockIdx.x*message_size, message_size, pe);
  }
}
...
int main()
{
  ...
  std::cout << "my_pe "<< my_pe << " n_pes "<< n_pes <<std::endl;
  ...
  for(int i=0; i<7; i++)
  {
      ...
      for(int j=0; j<num_round; j++)
      {
          nvshmem_barrier_all();
          timer.Start();
          int_thread<<<numBlock, numThread>>>(size, num_messages, message_size[i]/sizeof(int), rand_pe, rand_loc, remote_buffer, local_buffer );
  ...
  for(int i=0; i<7; i++)
  {
      ...
      for(int j=0; j<num_round; j++)
      {
          nvshmem_barrier_all();
          timer.Start();
          int_warp<<<numBlock, numThread>>>(size, num_messages, message_size[i]/sizeof(int), rand_pe, rand_loc, remote_buffer, local_buffer );
          cudaDeviceSynchronize();
          nvshmem_barrier_all();
          timer.Stop();
          totaltime = totaltime + timer.ElapsedMillis();
  ...
  for(int i=0; i<7; i++)
  {
      ...
      for(int j=0; j<num_round; j++)
      {
          nvshmem_barrier_all();
          timer.Start();
          int_block<<<numBlock, numThread>>>(size, num_messages, message_size[i]/sizeof(int), rand_pe, rand_loc, remote_buffer, local_buffer );
          cudaDeviceSynchronize();
          nvshmem_barrier_all();
          timer.Stop();
          totaltime = totaltime + timer.ElapsedMillis();
